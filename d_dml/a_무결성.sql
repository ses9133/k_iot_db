...
/*
    ...
    2) 참조 무결성
		: 데이터 간의 관계를 정의할 때, 한 테이블이 다른 테이블의 값을 참조하는 경우
          참조 값이 반드시! 존재해야함을 뜻함
		- 외래키(FK)를 통해 구현
        EX) 학생 - 수강, 고객 - 주문, 게시글 - 댓글 등..('해당 데이터에서' - '값을 참조')
        
    3) 도메인 무결성
		: 테이블의 각 필드(열, 컬럼)가 정의된 도메인에 맞는 값을 가져야 함
        - 데이터 타입, 포맷, 범위 등이 적절하게 유지
        - 데이터 타입 지정, unsigned, 제약 조건(CHECK) 등을 통해 구현
        EX) 은행 계좌에 입금되는 금액 - 양의 정수만 허용 (unsigned)
			점수에 입력되는 숫자 - 0 에서 100 사이의 정수 (CHECK)
